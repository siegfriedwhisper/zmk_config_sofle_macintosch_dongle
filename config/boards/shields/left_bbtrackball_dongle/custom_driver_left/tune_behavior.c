/*
 * "tune" behavior —— 接收器（dongle）下发参数的入口（左手 · 轨迹球）
 *
 * 接收器经 split 的 INVOKE_BEHAVIOR 命令下发，本半按 ID 段处理：
 *   0x01–0x3F → 轨迹球参数（本半）
 *   0x40–0x7F → 右手 TrackPoint 参数（不属本半，静默忽略）
...
LOG_MODULE_DECLARE(trackball_config, LOG_LEVEL_INF);
...
static int on_tune_binding_pressed(struct zmk_behavior_binding *binding,
    ...
    /* 不属于左手段（0x01–0x3F）的 ID 由 trackball_param_apply 返回 -1，静默忽略 */
    trackball_param_apply(id, value);
    return ZMK_BEHAVIOR_OPAQUE;
}
...
static int tune_init(const struct device *dev) {
    ARG_UNUSED(dev);
    LOG_INF("tune behavior ready (trackball param sink)");
    return 0;
}
